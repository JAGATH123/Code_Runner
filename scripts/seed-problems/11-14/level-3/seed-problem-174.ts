...
async function seedProblem174() {
  ...
  try {
    ...
    const problem174 = {
      ...
      difficulty: 'Hard',
      question: `The Nova Network bridge is overwhelmed with simultaneous translation, navigation, and defense requests. Can you engineer a complete composite AI system that demonstrates multiple inheritance, hybrid hierarchies, method overriding, and super() usage?

Your system must build a complete architecture demonstrating all advanced inheritance patterns working together in a unified intelligence framework.`,
      ...
      case_overview: `This final mission demonstrates composite AI architecture: multiple inheritance combining skills from different parents, hybrid inheritance creating multi-tier hierarchies, method overriding for refined behavior, and super() for extending functionality while preserving parent logic.`,
      case_explanation: `Build Navigator with navigate(name) and Analyst with analyze(name). Create MultiSkillAI(Navigator, Analyst). Build AIBase with boot(), PilotAI(AIBase) with fly(location), ScienceAI(AIBase) with research(target). Create HybridAI(PilotAI, ScienceAI) overriding report(). Add BaseSystem with startup(), AdvancedSystem using super().startup(). Read 3 inputs: AI name, flight location, research target. Call all methods demonstrating composite inheritance.`,
