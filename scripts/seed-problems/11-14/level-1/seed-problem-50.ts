...
In the inner loop, print " → Checking system " followed by the system number.`,
      ...
      metadata: {
        ...
        test_protocol: 'Students should use nested for loops with range() to create hierarchical iterations'
      },
...
# Now you try this for our task`,
      case_explanation: `Nested loops with range() create hierarchical iterations where the inner loop completes fully for each outer loop iteration. The outer loop runs for each stage, and the inner loop runs completely for each stage iteration. Indentation is critical: 4 spaces for outer body, 8 spaces for inner loop, 12 spaces for inner body. Use input() twice for stages and systems, converting both to integers. Outer loop: for stage in range(1, stages + 1):. Print in outer (indented 4 spaces): print("Stage", stage). Inner loop (indented 4 spaces): for system in range(1, systems + 1):. Print in inner (indented 8 spaces): print(" → Checking system", system). Note the arrow symbol (→) and leading space in the system print. The inner loop executes fully for each outer iteration.`,
