  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const problemEndpoint = levelProblemMap[resolved.level_number];
        if (!problemEndpoint) {
          notFound();
        }

        // Fetch Code Convergence Ultimate Challenge from database
        const response = await fetch(`/api/problems/${problemEndpoint}`);
