        ...
        if HAS_JS and text and text.strip():
            try:
                ...
                if not any(skip in clean_text for skip in skip_messages):
                    # Use direct JavaScript execution - more reliable than object creation
                    escaped_text = clean_text.replace('\', '\\').replace("'", "\'").replace('
', '\n')
                    js_code = f"window.parent.postMessage({{type: 'pygame-console', message: '{escaped_text}'}}, '*')"
                    js.eval(js_code)
            except Exception as e:
                # Print error for debugging
                self.original_stdout.write(f"[JSConsoleWriter Error: {e}]\n")
                self.original_stdout.flush()
